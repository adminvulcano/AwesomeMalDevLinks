...
_TLDR: Presenting DInvoke, a new API in SharpSploit that acts as a dynamic replacement for PInvoke. Using it, we show how to dynamically invoke unmanaged code from memory or disk while avoiding API Hooking and suspicious imports._
...
The section above shows how you would use Delegates and the DInvoke API. But how do you obtain the address for a function in the first place? The answer to that question really is: _however you would like_. But, to make that process easier, we have provided a suite of tools to help you locate and call code using a variety of mechanisms.
...
![_config.yml](https://thewover.github.io/images/DInvoke/4_Resolve.png)

## Why DInvoke?

DInvoke was built to allow you (the offensive tool developer) choice in not just _what_ code you execute but _how_ you execute it.
...
![_config.yml](https://thewover.github.io/images/DInvoke/theres-always-room-for-one-more-28316601.png)
...
![_config.yml](https://thewover.github.io/images/Manual_Map/ManMap_4.PNG)
...
![_config.yml](https://thewover.github.io/images/DInvoke/3_Syscall.png)
...
While Module Overloading is covert in that it hides a module in memory backed by a legitimate file on-disk, it does generate a modload event for the decoy file that backs the memory. Since that file is randomly chosen (and will not be one already loaded), it will probably not be a module that is _normally_ loaded by the process and is therefore anomalous.
...
![_config.yml](https://thewover.github.io/images/DInvoke/ModuleLoadCorrelation_pre.png)

Now, after injecting a .NET Assembly into the process, you can see that various .NET runtime DLLs were loaded into it.

![_config.yml](https://thewover.github.io/images/DInvoke/ModuleLoadCorrelation_post.png)
...
![_config.yml](https://thewover.github.io/images/DInvoke/MCpmBiUD.jpg_large.jpg)
...
![_config.yml](https://thewover.github.io/images/DInvoke/SilkETW_SharpSploit_Yara.png)
...
![_config.yml](https://thewover.github.io/images/DInvoke/Fermion_SharpSploit_MapModuleToMemory.png)
