...
## Finding a Signed Driver that’s not on the Microsoft Blocklist [Permalink](https://g3tsyst3m.com/byovd/BYOVD-and-Looting-LSASS-in-the-Modern-EDR-Era/\#finding-a-signed-driver-thats-not-on-the-microsoft-blocklist "Permalink")
...
![image](https://github.com/user-attachments/assets/0640b218-3619-47cc-a4d6-d55ff14c6f2a)![image](https://github.com/user-attachments/assets/61ad5e67-a9fd-4e24-9f83-ff0ce8ec423b)![image](https://github.com/user-attachments/assets/8a921097-5b96-4631-b0ad-83e287b13137)![image](https://github.com/user-attachments/assets/250862e9-05db-4ed3-a7a0-5258d4aff722)
...
![image](https://github.com/user-attachments/assets/8e6aec82-16ce-4b5e-a58f-e6d14a298418)
...
## The Inner workings of PDFWKRNL.sys [Permalink](https://g3tsyst3m.com/byovd/BYOVD-and-Looting-LSASS-in-the-Modern-EDR-Era/\#the-inner-workings-of-pdfwkrnlsys "Permalink")

**Step 1 — Tracing IOCTL 0x80002014 through the dispatch table**

We know from KDU’s previously researched info that the IOCTL code we are concerned with is **0x80002014**. Let’s start with **sub\_140001000**, which calculates both the IOCTL and the function that should be called based on a number of calculations we will explore below. Long story short: the IOCTL value is not in cleartext when reversing this driver 😸

Let’s start with how did we arrive at the **0x8000** value? Here’s how it works:

Windows IOCTL codes are structured 32-bit values built by the **CTL\_CODE macro**:
...
![image](https://github.com/user-attachments/assets/4a730dcc-cdd6-4ad1-9136-f70d3c356bd3)

**So again…How did we arrive at value 0x8000 for the start of our IOCTL code?**

Microsoft reserves device types below **0x8000** for official Windows device types (FILE\_DEVICE\_DISK, FILE\_DEVICE\_KEYBOARD, FILE\_DEVICE\_UNKNOWN, etc.).
...
![image](https://github.com/user-attachments/assets/3908a492-1a62-4810-8cc5-12122602ef19)
...
![image](https://github.com/user-attachments/assets/369b1257-7ce3-4764-bfe4-343883ae12cd)
...
## Code for the PPL Removal / Exploit Harness for PDFWKRNL.sys [Permalink](https://g3tsyst3m.com/byovd/BYOVD-and-Looting-LSASS-in-the-Modern-EDR-Era/\#code-for-the-ppl-removal--exploit-harness-for-pdfwkrnlsys "Permalink")
...
![image](https://github.com/user-attachments/assets/104c5429-31ca-4b24-b5fe-8d0c6f8bb6d4)
...
![image](https://github.com/user-attachments/assets/f49f882a-59da-441a-9303-f65396db1731)
...
![image](https://github.com/user-attachments/assets/80216119-ebcd-42f2-8ea1-3f4f763ba9c5)

![image](https://github.com/user-attachments/assets/f059d358-0d03-4ceb-a2ad-c713277513fe)
...
## Code demo for Disabling PPL protection in LSASS [Permalink](https://g3tsyst3m.com/byovd/BYOVD-and-Looting-LSASS-in-the-Modern-EDR-Era/\#code-demo-for-disabling-ppl-protection-in-lsass "Permalink")

Let’s see it in action!

Notice first how LSASS is currently protected:

![image](https://github.com/user-attachments/assets/96f620a2-e30c-48b2-994c-217c976c747e)

**Now we run our code:**

![image](https://github.com/user-attachments/assets/1f269199-1736-4b98-93b4-7e17ba3717c5)

Close out and go back in to System Informer:

![image](https://github.com/user-attachments/assets/08c30d16-4055-4999-bd13-50a926eea679)

We’re ready to proceed with Part 2 of this post - Dumping LSASS!

# Process Cloning and In-Memory Minidump Interception using Callbacks and XOR Encryption [Permalink](https://g3tsyst3m.com/byovd/BYOVD-and-Looting-LSASS-in-the-Modern-EDR-Era/\#process-cloning-and-in-memory-minidump-interception-using-callbacks-and-xor-encryption "Permalink")
...
## Headers, Globals, and the XOR Key [Permalink](https://g3tsyst3m.com/byovd/BYOVD-and-Looting-LSASS-in-the-Modern-EDR-Era/\#headers-globals-and-the-xor-key "Permalink")
...
## The NtCreateProcessEx Typedef [Permalink](https://g3tsyst3m.com/byovd/BYOVD-and-Looting-LSASS-in-the-Modern-EDR-Era/\#the-ntcreateprocessex-typedef "Permalink")
...
## XOR Obfuscation [Permalink](https://g3tsyst3m.com/byovd/BYOVD-and-Looting-LSASS-in-the-Modern-EDR-Era/\#xor-obfuscation "Permalink")
...
## The Worker Thread [Permalink](https://g3tsyst3m.com/byovd/BYOVD-and-Looting-LSASS-in-the-Modern-EDR-Era/\#the-worker-thread "Permalink")
...
## The Callback Routine and Intercepting MiniDump Writes [Permalink](https://g3tsyst3m.com/byovd/BYOVD-and-Looting-LSASS-in-the-Modern-EDR-Era/\#the-callback-routine-and-intercepting-minidump-writes "Permalink")
...
## Putting It All Together [Permalink](https://g3tsyst3m.com/byovd/BYOVD-and-Looting-LSASS-in-the-Modern-EDR-Era/\#putting-it-all-together "Permalink")
...
## Evasion Summary [Permalink](https://g3tsyst3m.com/byovd/BYOVD-and-Looting-LSASS-in-the-Modern-EDR-Era/\#evasion-summary "Permalink")
...
## Code Demo! [Permalink](https://g3tsyst3m.com/byovd/BYOVD-and-Looting-LSASS-in-the-Modern-EDR-Era/\#code-demo "Permalink")

![image](https://github.com/user-attachments/assets/f8e61426-1448-4d06-954a-07767e8d3634)

And the encrypted file:

![image](https://github.com/user-attachments/assets/3d730761-4dcf-44bc-b52a-2eab13e94ae4)

okay, let’s decrypt this thing!

![image](https://github.com/user-attachments/assets/9fa293e8-f7f6-4d91-a1e5-12a35a3b0837)

![image](https://github.com/user-attachments/assets/99964278-efaa-42ab-b34a-9f86d06bfd2e)
...
## 🛡️ So, How do I defend against This? 🛡️ [Permalink](https://g3tsyst3m.com/byovd/BYOVD-and-Looting-LSASS-in-the-Modern-EDR-Era/\#%EF%B8%8F-so-how-do-i-defend-against-this-%EF%B8%8F "Permalink")
...
## 🎁 **_Bonus Content for Members! (All Tiers)_** 🎁 [Permalink](https://g3tsyst3m.com/byovd/BYOVD-and-Looting-LSASS-in-the-Modern-EDR-Era/\#-bonus-content-for-members-all-tiers- "Permalink")

Additional TPwSav.sys BYOVD driver + code! [TPwSav.sys Source Code on Ko-fi](https://ko-fi.com/s/aeb093ef9e)

![image](https://github.com/user-attachments/assets/3dd53e0b-8f62-41c7-8745-1fd1174cf04e)

## **_ANY.RUN Results_** [Permalink](https://g3tsyst3m.com/byovd/BYOVD-and-Looting-LSASS-in-the-Modern-EDR-Era/\#anyrun-results "Permalink")
...
[Twitter](https://twitter.com/intent/tweet?text=BYOVD+and+Looting+LSASS+in+the+Modern+EDR+Era%20https%3A%2F%2Fg3tsyst3m.com%2Fbyovd%2FBYOVD-and-Looting-LSASS-in-the-Modern-EDR-Era%2F "Share on Twitter") [Facebook](https://www.facebook.com/sharer/sharer.php?u=https%3A%2F%2Fg3tsyst3m.com%2Fbyovd%2FBYOVD-and-Looting-LSASS-in-the-Modern-EDR-Era%2F "Share on Facebook") [LinkedIn](https://www.linkedin.com/shareArticle?mini=true&url=https%3A%2F%2Fg3tsyst3m.com%2Fbyovd%2FBYOVD-and-Looting-LSASS-in-the-Modern-EDR-Era%2F "Share on LinkedIn")

#### Leave a comment

## You may also enjoy

![](https://g3tsyst3m.com/assets/images/quic.png)

## [Building an Encrypted C2 Implant Using QUIC](https://g3tsyst3m.com/encrypted%20shell/Building-an-Encrypted-C2-Implant-Using-QUIC/)
...
Hey everyone! If you’ve been following the blog for a while, you’ll likely recall my python driven C2 series. Today we’re doing something a little different...

![](https://g3tsyst3m.com/assets/images/fud.png)

## [Creative approaches to coding FUD Stagers](https://g3tsyst3m.com/fud/Creative-approaches-to-coding-FUD-Stagers/)
...
I have had several discussions over the years with folks on tackling EDR bypass as it pertains to fully undetected (FUD) code. In my opinion, there isn’t re...

![](https://g3tsyst3m.com/assets/images/MOTW.png)

## [Gaining Initial Access and Outsmarting SmartScreen](https://g3tsyst3m.com/initial%20access/Gaining-Initial-Access-and-Outsmarting-SmartScreen/)
...
I’ve had a number of people ask me what approaches I have taken to secure that initial foothold on a machine/network during a pentest engagement. Well, the ...
